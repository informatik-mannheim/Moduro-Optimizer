...
import hs.mannheim.moduro.automation.cc3d.simulation.manager.cc3d.model.ParameterDumpExecConfig;
...
public class ParameterDumpReaderImpl implements ParameterDumpReader {
    ...
    public ParameterDump parseParamDump(File parameterDumpFile) {
        try {
            ...
            if (StringUtils.endsWithIgnoreCase(parameterDumpFile.getName(), "json")) {
                System.out.println("Parsing json file. Converting Json to Instance of ParameterDump");
                Gson gson = new Gson();
                try {
                    JsonReader jsonReader = new JsonReader(new FileReader(parameterDumpFile));
                    return (ParameterDump) gson.fromJson(jsonReader, ParameterDump.class);
