...
public class CmaesTest {
    ...
    public void cmaesTest() {
        ...
        // Basisparameter für CMAES Framework
        int maxIterations = 1999999999;
        double stopFitnessDouble = 1.00;
        boolean isActiveCma = true;
        int digitalOnly = 1; //diagonalOnly - Number of initial iterations, where the covariance matrix remains diagonal.
        int checkFeasableCount = 1;// checkFeasableCount - Determines how often new random objective variables are generated in case they are out of bounds.
        RandomGenerator randomGen = RandomGeneratorFactory.createRandomGenerator(new Random());
        Boolean generateStatistics = false;
        SimplePointChecker pointChecker = new SimplePointChecker(-1.0, -2.0);

        CMAESOptimizer cmaesOptimizer =
                new CMAESOptimizer(maxIterations, stopFitnessDouble, isActiveCma,
                        digitalOnly, checkFeasableCount, randomGen, generateStatistics,
                        pointChecker);

        // todo: Diese wird als FitnessFunktion mitgegeben
        // todo: hier CC3D Ausführen, Fitness-Werte im Working Dir für die aktuelle Simulation auslesen
        // todo: fitness entsprechend der Werte berechnen
         ...
         *
         * - Liest aktuelles Simulations WorkingDir aus
         * - Läd FitnessPlot.dat
         * - Analysiert Werte und berechnet eigene Fitness
         */
        ...
        File moduroPythonProjectDirFile = new File("C:\\Users\\Station\\PycharmProjects\\Moduro-CC3D\\Simulation\\RunJsonCc3D.py");
         ...
         * Interne FitnessFunktion
         *
         * - Baut ParameterDump auf Basis von Werten vom CMAES-Optimizer
         * - Startet neue CC3D Simulation auf Basis der Parameter
         * - Wertet Ergebnisse aus dem CC3D Working Direktory für die Simulation aus
         * - Gibt Ergebnis als Fitnesswert an CMAES-Optimizer zurück
         * - CMAES-Optimizer berechnet neu Werte
        ...
        ObjectiveFunction internalFitnessFunction = new ObjectiveFunction(new MultivariateFunction() {
            ...
            public double value(double[] doubles) {
                ...
                ParameterDump optimizedValuesParameterDump
                        = getOptimizedValuesParameterDump(finalBaseParameterDump, doubles);
            ...
            private double calculateInternalFitness(Collection<Double> simulationFitnessValues) {
                // todo: hier die Fitnessfunktion auf Basis der Daten aus der Datei
                // "FitnessPlot.dat" .
                return 1.0;
            }

            /***
             * Lädt die aktuelle FitnessPlot.dat im cc3dWorkingDir
             * Extrahiert die Fitnesswerte aus dem File in eine Liste
        ...
        try {
            ...
            double[] initialGuessValues = new double[]{modelAdhEnergy, modelAdhFactor, cellStemGrothVolumePerDay, cellStemNecrosisProb,
                    cellStemnutrientRequirement, cellStemSurFit, cellStemVolFit, cellBasalGrothVolumePerDay, cellBasalNecrosisProb,
            ...
            // todo: sigma muss gesetzt sein, wie wird es verarbeitet und welche werte setzt man?
            ...
            double[] upperBounds = {1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000};
            ...
            PointValuePair optimize = cmaesOptimizer.optimize(internalFitnessFunction, populationSize,
                    goalTypeMaximize, initialGuess, sigma
                    , simpleBounds, maxEval);

            System.out.println("Done optimizing values");

            // todo: neue ParameterDump
            // todo Setzen der optimierten Werte
            // todo: originale parameterdump nehmen und die Werte entsprechend der Reihenfolge ersetzen
